...
static void generate_ghost_particles(const double domain[4], double dx, std::vector<vec2>& ghosts){
    double xmin = domain[0], ymin = domain[1], xmax = domain[2], ymax = domain[3];
    int layers = 3;
    double pad = layers*dx;

    for(int L=1; L<=layers; L++){
        double gy = ymin - (L-0.5)*dx;
        for(double gx = xmin-pad+0.5*dx; gx <= xmax+pad-0.5*dx+1e-9; gx += dx) ghosts.push_back(vec2{gx, gy});
    }
    for(int L=1; L<=layers; L++){
        double gy = ymax + (L-0.5)*dx;
        for(double gx = xmin-pad+0.5*dx; gx <= xmax+pad-0.5*dx+1e-9; gx += dx) ghosts.push_back(vec2{gx, gy});
    }
    for(int L=1; L<=layers; L++){
        double gx = xmin - (L-0.5)*dx;
        for(double gy = ymin+0.5*dx; gy <= ymax-0.5*dx+1e-9; gy += dx) ghosts.push_back(vec2{gx, gy});
    }
    for(int L=1; L<=layers; L++){
        double gx = xmax + (L-0.5)*dx;
        for(double gy = ymin+0.5*dx; gy <= ymax-0.5*dx+1e-9; gy += dx) ghosts.push_back(vec2{gx, gy});
    }
}
...
int main(int argc, char** argv){
    ...
    generate_ghost_particles(sp.domain, sp.dx, ghosts);
    int n_ghost = (int)ghosts.size();
    int n = n_fluid + n_ghost;
    ...
    for(int i=0;i<n_ghost;i++){
        position_array[n_fluid+i] = ghosts[i];
        velocity_array[n_fluid+i] = vec2{0,0};
        acceleration_array[n_fluid+i] = vec2{0,0};
    }
    ...
    printf("done: n_fluid=%d n_ghost=%d steps=%lld t=%.6f frames=%d\n", n_fluid, n_ghost, step, t, frame);
    return 0;
}
