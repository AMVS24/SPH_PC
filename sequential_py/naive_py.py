...
class ball():
    def __init__(self, radius, number, p=(0, 0), v=(0, 0), a=(0, 0), m=20, batch=main_batch, name=None, density=1):
        ...
        # Quad (axis-aligned square) instead of a circle, anchored at its centre so
        # that `position` still refers to the particle centre.
        self.sprite = pyglet.shapes.Rectangle(*(ORIGIN + self.p), 2 * radius, 2 * radius,
                                              color=(0, 0, 255), batch=batch)
        self.sprite.anchor_position = (radius, radius)
    ...
    def update_parameters(self, dt):
        ...
        self.sprite.position = tuple(ORIGIN + self.p)
...
ball_list = []
random.seed(42)                       # fixed seed so every run spawns the same fluid
if (len(sys.argv) > 1):
...
def force_calc(particle):
    ...
    for j in ball_list:
        ...
        q = distance / Sim.smoothening_radius
        diffs = particle.p - j.p                                                # ri-rj
        v_diff = particle.v - j.v                                               # vi-vj
        dot_prod = np.dot(v_diff, diffs)

        if dot_prod < 0:                                                        # if rel_vel is opposite to rel_direction, only then we apply visc
            mu = Sim.smoothening_radius * dot_prod / (
                        q ** 2 * Sim.smoothening_radius ** 2 + 0.0001 * Sim.smoothening_radius ** 2)
            avg_density = (particle.density + j.density) / 2
            c = 150
            # Calculate the full viscosity scalar field (Pi_ij)
            pi = (-0.1 * c * mu) / (avg_density + epsilon)

            # kernel_gradient() carries the opposite sign convention to the vectorised
            # kernel_gradient_vectorized(), so `direction` (j -> i) is passed here where
            # the vectorised code passes diffs (i -> j). Same resulting vector.
            grad_value = kernel_gradient(q, direction)

            # Calculate the final viscosity force vector
            viscosity_force += -pi * grad_value

    # Add the viscosity force to the total acceleration
    return -force + viscosity_force
...
def update_forces():
    for i in ball_list:
        i.a = np.array((0, -40), dtype=float)      # gravity, mirrors Sim.acceleration_array += np.array((0, -40))
        i.a += force_calc(i)
